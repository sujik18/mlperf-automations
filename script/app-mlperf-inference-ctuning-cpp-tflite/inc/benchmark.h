...
public:
  ...
  const std::string available_images_file =
      getenv_s("MLC_DATASET_PREPROCESSED_IMAGES_LIST");
  const bool skip_internal_preprocessing =
      (getenv_opt_s("MLC_DATASET_COMPRESSED", "off") == "off");
  const std::string result_dir = getenv_s("MLC_MLPERF_OUTPUT_DIR");
  const std::string input_layer_name =
      getenv_s("MLC_ML_MODEL_INPUT_LAYER_NAME");
  const std::string output_layer_name =
      getenv_s("MLC_ML_MODEL_OUTPUT_LAYER_NAME");
  ...
  BenchmarkSettings(enum MODEL_TYPE mode = MODEL_TYPE::LITE) {
    ...
    if (getenv_opt_s("MLC_HOST_USE_ALL_CORES", "no") != "yes") {
      ...
      if (getenv_i("MLC_HOST_CPU_TOTAL_CORES") &&
          getenv_i("MLC_HOST_CPU_THREADS_PER_CORE")) {
        _number_of_threads = getenv_i("MLC_HOST_CPU_TOTAL_CORES") /
                             getenv_i("MLC_HOST_CPU_THREADS_PER_CORE");
      }
    }
    // Print settings
    ...
    std::cout << "Run time preprocessing: " << !skip_internal_preprocessing
              << std::endl;
