...
def preprocess(i):
    ...
    CPATH = env.get('+CPATH', [])
    env['MLC_C_INCLUDE_PATH'] = " -I".join([f"""{q}{path}{q}""" for path in [" "] + env.get('+C_INCLUDE_PATH', []) + CPATH])

    env['MLC_C_INCLUDE_PATH'] = " -I".join(f"""{q}{path}{q}""" for path in [" "] + env.get('+C_INCLUDE_PATH', []) + CPATH)

    env['MLC_CPLUS_INCLUDE_PATH'] = " -I".join(f"""{q}{path}{q}""" for path in [" "] + env.get('+CPLUS_INCLUDE_PATH', []) + CPATH)

    env['MLC_F_INCLUDE_PATH'] = " -I".join(f"""{q}{path}{q}""" for path in [" "] + env.get('+F_INCLUDE_PATH', []) + CPATH)
    ...
    env['MLC_LD_LIBRARY_PATH'] = " -L".join(f"""{q}{path}{q}""" for path in [" "] + env.get('+LD_LIBRARY_PATH', []))
