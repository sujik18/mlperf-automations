...
enum MODEL_TYPE { LITE, TF_FROZEN };
...
public:
  ...
  const bool normalize_img =
      getenv_s("CK_ENV_TENSORFLOW_MODEL_NORMALIZE_DATA") == "YES";
  const bool subtract_mean =
      getenv_s("CK_ENV_TENSORFLOW_MODEL_SUBTRACT_MEAN") == "YES";
  const char *given_channel_means_str =
      getenv("MLC_ML_MODEL_GIVEN_CHANNEL_MEANS");
...
public:
  ...
  bool get_next_batch() {
    ...
    if (_settings->full_report || batch_number % 10 == 0)
      std::cout << "\nBatch " << batch_number << " of "
                << _settings->batch_count << std::endl;
...
inline void finish_benchmark(const BenchmarkSession &s) {
  // Store metrics
  /* store_value_f(X_VAR_TIME_SETUP, "setup_time_s",
   xopenme_get_timer(X_TIMER_SETUP)); store_value_f(X_VAR_TIME_TEST,
   "test_time_s", xopenme_get_timer(X_TIMER_TEST));
   store_value_f(X_VAR_TIME_IMG_LOAD_TOTAL, "images_load_time_total_s",
   s.total_load_images_time()); store_value_f(X_VAR_TIME_IMG_LOAD_AVG,
   "images_load_time_avg_s", s.avg_load_images_time());
   store_value_f(X_VAR_TIME_CLASSIFY_TOTAL, "prediction_time_total_s",
   s.total_prediction_time()); store_value_f(X_VAR_TIME_CLASSIFY_AVG,
   "prediction_time_avg_s", s.avg_prediction_time());
...
template <typename L> void measure_setup(L &&lambda_function) {
  // xopenme_clock_start(X_TIMER_SETUP);
  lambda_function();
  // xopenme_clock_end(X_TIMER_SETUP);
}

template <typename L> void measure_prediction(L &&lambda_function) {
...
public:
  ImageData(const BenchmarkSettings *s)
      : StaticBuffer(s->image_size * s->image_size * s->num_channels *
                         (s->skip_internal_preprocessing ? sizeof(float)
                                                         : sizeof(uint8_t)),
                     s->images_dir) {}
...
public:
  ...
  void save_results(const std::vector<std::string> &batch_images) override {
    ...
    for (auto image_file : batch_images) {
      _out_converter->convert(_out_ptr + image_offset + probe_offset,
                              _out_data.get());
