...
def preprocess(i):
    ...
    if env.get('MLC_ML_MODEL_NAME', '') == 'retinanet':
        if env.get('MLC_DATASET_QUANTIZE', '') == '1':
            if env.get('MLC_QAIC_MODEL_RETINANET_IMAGE_SCALE', '') != '':
                env['MLC_DATASET_QUANT_SCALE'] = env['MLC_QAIC_MODEL_RETINANET_IMAGE_SCALE']
            if env.get('MLC_QAIC_MODEL_RETINANET_IMAGE_OFFSET', '') != '':
                env['MLC_DATASET_QUANT_OFFSET'] = env['MLC_QAIC_MODEL_RETINANET_IMAGE_OFFSET']
...
def postprocess(i):
    ...
    if env["MLC_DATASET_TYPE"] == "validation":
        env['MLC_DATASET_ANNOTATIONS_DIR_PATH'] = os.path.join(
            env['MLC_DATASET_PREPROCESSED_PATH'], "annotations")
        env['MLC_DATASET_ANNOTATIONS_FILE_PATH'] = os.path.join(
            env['MLC_DATASET_ANNOTATIONS_DIR_PATH'], "openimages-mlperf.json")
