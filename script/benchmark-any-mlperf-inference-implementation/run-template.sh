...
find_ss_performance_cmd='mlcr generate-run-cmds,inference,_find-performance \
--model=$model --implementation=$implementation --device=$device --backend=$backend \
...
readme_cmd_single='mlcr generate-run-cmds,inference,_populate-readme --scenario=$scenario \
--model=$model --implementation=$implementation --device=$device --backend=$backend \
--category=$category --division=$division  --quiet  \
--skip_submission_generation=yes --execution-mode=valid ${POWER_STRING} ${EXTRA_RUN_ARGS} ${EXTRA_ARGS}'
...
tflite_accuracy_cmd='mlcr run,mobilenet-models,_tflite,_accuracy-only$extra_tags \
--adr.compiler.tags=gcc \
${extra_option} \
 ${EXTRA_ARGS}'

tflite_performance_cmd='mlcr run,mobilenet-models,_tflite,_performance-only$extra_tags \
${POWER_STRING} \
--adr.compiler.tags=gcc \
${extra_option} \
 ${EXTRA_ARGS}'

tflite_readme_cmd='mlcr run,mobilenet-models,_tflite,_populate-readme$extra_tags \
${POWER_STRING} \
--adr.compiler.tags=gcc \
${extra_option} \
 ${EXTRA_ARGS}'
