...
if [[ ${MLC_INTEL_IPEX_RESNET50_PATCH} == "yes" ]]; then
  bash ${MLC_TMP_CURRENT_SCRIPT_PATH}/apply_intel_resnet50_patch.sh
  test "$?" -eq 0 || exit "$?"

elif [[ ${MLC_INTEL_IPEX_RETINANET_PATCH} == "yes" ]]; then
  bash ${MLC_TMP_CURRENT_SCRIPT_PATH}/apply_intel_retinanet_patch.sh
  test "$?" -eq 0 || exit "$?"

elif [[ ${MLC_INTEL_IPEX_3D_UNET_PATCH} == "yes" ]]; then
  cd third_party/mkl-dnn
  git fetch --tags && git checkout v2.7
  test "$?" -eq 0 || exit "$?"
  cd ../../
  bash ${MLC_TMP_CURRENT_SCRIPT_PATH}/apply_intel_3d-unet_patch.sh
  test "$?" -eq 0 || exit "$?"

elif [[ ${MLC_INTEL_IPEX_DLRM_V2_PATCH} == "yes" ]]; then
